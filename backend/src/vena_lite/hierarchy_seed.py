...
# (dim, member, parent_or_None, rollup_op, ordinal, display_name_or_None)
HierarchyRow = tuple[str, str, str | None, str, int, str | None]


def _flat(dim: str, members: list[str]) -> list[HierarchyRow]:
    return [(dim, m, None, "sum", i, None) for i, m in enumerate(members)]


def _account_hierarchy() -> list[HierarchyRow]:
    rows: list[HierarchyRow] = [("account", "Total_PnL", None, "sum", 0, None)]
    rows += [("account", a, "Total_PnL", "sum", i + 1, None) for i, a in enumerate(ACCOUNTS)]
    return rows


def _entity_hierarchy() -> list[HierarchyRow]:
    rows: list[HierarchyRow] = [("entity", "Worldwide", None, "sum", 0, None)]
    rows += [("entity", e, "Worldwide", "sum", i + 1, None) for i, e in enumerate(ENTITIES)]
    return rows
...
def _period_hierarchy() -> list[HierarchyRow]:
    ...
    """
    rows: list[HierarchyRow] = [("period", "2026-FY", None, "sum", 0, None)]
    quarters = ["2026-Q1", "2026-Q2", "2026-Q3", "2026-Q4"]
    for i, q in enumerate(quarters):
        rows.append(("period", q, "2026-FY", "sum", i + 1, None))
    for i, p in enumerate(PERIODS):
        month = int(p.split("-")[1])
        q = quarters[(month - 1) // 3]
        rows.append(("period", p, q, "sum", 100 + i, None))
    return rows
